...
const argv = yargs(hideBin(process.argv))
  ...
  .option("find", { alias: "f", default: "com.fasterxml.jackson" })
  .boolean("dot")
  .alias("d", "dot")
  ...
  .describe(
    "left",
    "force graph building from left-to-right, instead of top-to-bottom"
  )
  .help().argv;

// matchers, test via https://regex101.com/
const TREE_SYMBOLS = /^[| \-+\\]+/i;
const DEPENDENCY = /((Task|project) )?([a-z:0-9.\-]{10,}(\{[^}]+})?)/i;
...
const projectNodes = [];
const taskNodes = [];
const styleTaskNode = (node, tasks) => {
  const isTask =
    node.dependency.includes("Task ") || node.dependency.includes("Classpath");

  if (isTask && !tasks.includes(node)) tasks.push(node);
};
const styleProjectNode = (node, projects) => {
...
const styledNode = (node, projects, tasks) => {
  styleProjectNode(node, projects);
  styleTaskNode(node, tasks);
};
...
if (isDot) {
  ...
  console.log(
    '    graph [pad="0.5", nodesep="1", ranksep="2"]; splines="false"; node[shape = rectangle];'
  );
  /* Change Graph orientation. */
  if (argv.left) console.log(`    rankdir=LR;`);
}

while ((rawLine = liner.next())) {
  const line = rawLine.toString("utf-8");
  const isFound = line.includes(argv.find);
    ...
    if (isDot) {
      ...
      for (const node of stack) {
        ...
        if (isUnKnownEdge) {
          ...
          if (delimiter !== " -> ") {
            // is it a first run?
            if (prevNode.dependency !== "") {
              // is it not a "empty root" node?
